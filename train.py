...
def init_dataset_kwargs(data, data_name, **kwargs):
    if data_name == "adni":
        class_name = "training.dataset.AdniMRIDataset3D"
        ...
        raise click.ClickException(
            f"--data_name: {data_name} dataloader not implemented"
        )
    try:
        ...
        # dataset_kwargs.labels_dim = dataset_obj.label_dim
...
@click.option(
    "--data", help="Training data", metavar="[ZIP|DIR|CSV]", type=str, required=True
)
@click.option(
    "--data_name", help="Dataset name", metavar="STR", type=str, required=True
)
@click.option(
    "--wandb_name",
...
def main(**kwargs):
    ...
    c.training_set_kwargs, dataset_name = init_dataset_kwargs(
        data=opts.data, data_name=opts.data_name
    )
